...
@author: Hongjian Tang(NUS/SEU) Correspondence and requests for materials should be addressed to chejj@nus.edu.sg
"""
...
#####  Atom types of Curated COFs by lammps_interface
# data_curated_atom = pd.read_csv('Atom_Curated.csv') 
# data_curated_atom.drop(columns='Unnamed: 0',inplace=True)

#####  Bond types of Curated COFs by lammps_interface
# data_curated_bond = pd.read_csv('Bond_Curated.csv') 
# data_curated_bond.drop(columns='Unnamed: 0',inplace=True)

lcd_curated = data_curated.iloc[:,3]
pld_curated = data_curated.iloc[:,4]
lfpd_curated = data_curated.iloc[:,5]
vf_curated = data_curated.iloc[:,6]
pv_curated = data_curated.iloc[:,7]
povf_curated = data_curated.iloc[:,8]
ponvf_curated = data_curated.iloc[:,9]
popv_curated = data_curated.iloc[:,10]
ponpv_curated = data_curated.iloc[:,11]
vsa_curated  = data_curated.iloc[:,12]
gsa_curated  = data_curated.iloc[:,13]
nvsa_curated  = data_curated.iloc[:,14]
ngsa_curated  = data_curated.iloc[:,15]
den_curated  = data_curated.iloc[:,17]
...
# Selected COF features
X_curated = Geo_curated
